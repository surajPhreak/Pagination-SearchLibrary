...
// exit;
	...
	$table_heading_name=array('Name','Email','Phone','Gender');
	...
	{   
	    ...
	    $value=array(array('string','mock_test_tbl.mock_test_tbl','name','name,id,email, phone, gender','id'));
	    $query_data=array();
	    ...
	    $table1_to=array("mock_test_tbl.mock_test_tbl","login_db.mock_test_tbl");
        	...
        	$extra_slots_entry= mysqli_prepared_query($connection_mock_chat,$query,"ii",$params);
		        if($extra_slots_entry)
		        {
		            foreach ($extra_slots_entry as $val)
		            {
		                ...
		                $phoneNum=isset($val['phone'])?$val['phone']:'';
